...
class Flight:
    ...
    def __init__(self, ownername, passengers, luggage, origin, destination):
    ...
    def luggage_code(self):
        ...
        passLugg= len(self.__luggage)
        while passLugg !=0:
            code= ''
            for i in range(5):
                code+= letters[rn.randint(0,8)]+str(rn.randint(0,9))
            passLugg-=1
            luggcodes.append(code)
        return f'Los códigos para sus maletas son: {luggcodes}'
    ...
    def __repr__(self):
        return f'Flight({self.__ownername}, {self.__passengers}, {self.__luggage}, {self.__origin}, {self.__destination}, {repr(self.__date)}, {repr(self.__interface)}'
    
    def __str__(self):
        return f'Su vuelo de: {self.__origin} --- {self.__destination}, se ha registrado a nombre de: {self.__ownername}. {self.__date}. Con el siguiente equipaje: {self.__luggage}'
    
    
if __name__ == '__main__':
    l1= Luggage('Nicolás Macías', 50, 30, 20, 40)
    p1= Passenger('Nicolás Macías', 'M', 20)
    print(p1)
    print(l1)
    p2= Passenger('Lancelot Micelas', 'M', 32)
    l2= Luggage('Lancelot Micelas', 60, 80, 50, 10)
    print(p2)
    print(l2)
    p3= Passenger('Joshua Hernández', 'M', 17)
    l3= Luggage('Joshua Hernández', 50, 40, 30, 20)
    print(p3)
    print(repr(p3))
    p4= Passenger('Michelle Serret', 'F', 83)
    print(repr(p4))
    v1= Flight('Lancelot Micelas', [p1, p2], [l1, l2], 'Montevideo, Uruguay', 'Ciudad de México, México')
    print(v1.calculate_price())
    print(v1.generate_code())
    v1.assign_code()
    print(v1.luggage_code())
    print(repr(v1))
